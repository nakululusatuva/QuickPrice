...
/var/lib/quickprice/config/admin-account.json  quickprice      0600
...
single administrator username, scrypt-verified password, and TOTP factor
generated by `quickprice admin-credentials`. The username defaults to `admin`.
Keep the temporary plaintext password and TOTP seed in separate protected
records. Never copy either plaintext value into the web-managed files.

Install the emitted username, password verifier,
`password_change_required=true`, TOTP seed, and origin in the root-controlled
`/etc/quickprice/admin-auth.env`. On first start, QuickPrice atomically creates
`/var/lib/quickprice/config/admin-account.json` with mode 0600. Log in with the
temporary password and TOTP code, change the password immediately, then remove
the username, password-verifier, and change-required bootstrap assignments from
`admin-auth.env` and restart. The account file is authoritative after
bootstrap; retaining the old bootstrap verifier could restore the temporary
password if the account file were accidentally deleted.

Upgrades from administrator-key authentication are intentionally not
transparent. `QUICKPRICE_ADMIN_KEY_VERIFIER` is ignored. Preserve the existing
TOTP secret and use `quickprice admin-password-verifier --username NAME` to
create a password bootstrap without changing authenticator enrollment. Remove
the old key-verifier assignment and install the three emitted bootstrap
assignments beside the existing TOTP secret and origin. Do not copy an old key
verifier into `QUICKPRICE_ADMIN_PASSWORD_VERIFIER`. Existing administrator
sessions end with the restart. Quote API keys remain unchanged. For rollback,
restore the legacy environment from a protected backup because an older
release cannot read `admin-account.json`.
...
Back up `admin-account.json` with the database and managed instrument policy,
without copying it into a less protected location. If it is lost after the
one-time bootstrap assignments have been removed, generate a new administrator
bootstrap and TOTP enrollment rather than restoring the obsolete temporary
password verifier.
